...
public class MoveChecker {
	...
	public static MoveLegality isPseudoLegalMove(Move move, Position position) {
		...
		if (piece == PieceType.PAWN) {
			return MoveLegality.valueOf(isPseudoLegalPawnMove(move, position));
		} else if (piece == PieceType.KING) {
			if (isPseudoLegalCastleMove(move, position)) {
				return MoveLegality.LEGAL_MOVE;
			}
			return MoveLegality.valueOf(isPseudoLegalMoveByOffset(kingMoveOffsets, move, position));
		} else if (piece == PieceType.KNIGHT) {
			return MoveLegality.valueOf(isPseudoLegalMoveByOffset(knightMoveOffsets, move, position));
		} else if (piece == PieceType.BISHOP) {
			return MoveLegality.valueOf(isPseudoLegalMoveByDirection(bishopDirections, move, position));
		} else if (piece == PieceType.ROOK) {
			return MoveLegality.valueOf(isPseudoLegalMoveByDirection(rookDirections, move, position));
		} else if (piece == PieceType.QUEEN) {
			return MoveLegality.valueOf(isPseudoLegalMoveByDirection(queenDirections, move, position));
		} else {
			throw new AssertionError();
		}
	}

	private static boolean isPseudoLegalMoveByOffset(int[] offsets, Move move, Position position) {
	...
	private static boolean isPseudoLegalMoveByDirection(int[] directions, Move move, Position position) {
		int boundaryIndexFrom = toBoundaryIndex(move.getFrom());
	...
	private static boolean isPseudoLegalPawnMove(Move move, Position position) {
	...
	public static boolean isPseudoLegalCastleMove(Move move, Position position) {
		if (position.isWhiteToMove() && move.getFrom() == 4 && move.getTo() == 6) {
			return (position.canWhiteCastleKingside() && position.getPiece(4) == Piece.WHITE_KING
					&& position.getPiece(5) == Piece.EMPTY && position.getPiece(6) == Piece.EMPTY && position
					.getPiece(7) == Piece.WHITE_ROOK);
		}
		...
		if (!position.isWhiteToMove() && move.getFrom() == 60 && move.getTo() == 62) {
			return (position.canBlackCastleKingside() && position.getPiece(60) == Piece.BLACK_KING
					&& position.getPiece(61) == Piece.EMPTY && position.getPiece(62) == Piece.EMPTY && position
					.getPiece(63) == Piece.BLACK_ROOK);
		}
		...
		for (int index = 0; index < 64; ++index) {
			...
			if (srcSquare.isPiece() && srcSquare.isWhite() != isWhiteKing) {
				for (int kingIndex : kingIndices) {
					...
					if (isPseudoLegalMove(move, position).isLegal()) {
						return true;
					}
				}
			}
	...
	public static boolean isLegalMove(Move move, Position position) {
		if (!isPseudoLegalMove(move, position).isLegal()) {
			return false;
		}
		Position trialPosition = position.copy();
