...
public class SanWriter {
	...
	public String write(Position position, Move move)
			throws SanWritingException {
	...
	private void writeFrom() throws SanWritingException {
		...
		for (Iterator<Move> iterator = moveCandidates.iterator(); iterator
			...
			if (!MoveChecker.isPseudoLegalMove(moveCandidate, position)
					.isLegal()) {
				iterator.remove();
			}
		}
		if (moveCandidates.size() == 0) {
			throw new SanWritingException(
					"No piece can pseudo-legally do that move");
		}
		...
		for (Iterator<Move> iterator = moveCandidates.iterator(); iterator
				.hasNext();) {
			Move moveCandidate = iterator.next();
			if (!MoveChecker.isLegalMove(moveCandidate, position)) {
				iterator.remove();
			}
		}
		if (moveCandidates.size() == 1) {
	...
	private void writeMateSuffix() {
		Position position2 = position.copy();
		position2.makeMove(move);
